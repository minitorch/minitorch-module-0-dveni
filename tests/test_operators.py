...
def test_sigmoid(a: float, b: float) -> None:
    ...
    * It is  strictly increasing.
    """
    assert lte(0.0, sigmoid(a)) and lte(sigmoid(a), 1.0)
    assert_close(1 - sigmoid(a), sigmoid(neg(a)))
    assert_close(sigmoid(0), 0.5)
    if lte(a, b):
        assert lte(sigmoid(a), sigmoid(b))
...
@pytest.mark.task0_2
@given(small_floats)
def test_log(a: float) -> None:
    """Test that the logarithm of the exponent of a number is the number"""
